...
async function getDocuments(scope: ScopeKey, userId: number) {
  const baseWhere = scopeConfig[scope].where(userId);
  
  // Добавляем фильтр видимости:
  // - Ответственный видит все документы, где он назначен ответственным
  // - Остальные видят только документы, где они участвуют в цепочке
  const visibilityFilter = {
    OR: [
      { responsibleId: userId }, // Ответственный видит всё
      { authorId: userId }, // Автор видит свои документы
      {
        tasks: {
          some: {
            assigneeId: userId, // Участник видит документы из своей цепочки
          },
        },
      },
    ],
...
async function getScopeCounts(userId: number) {
  const entries = (Object.keys(scopeConfig) as ScopeKey[]).map(async (scope) => {
    const count = await prisma.document.count({ where: scopeConfig[scope].where(userId) });
    return [scope, count] as const;
  });
...
export default async function DashboardPage({
  ...
  const [documents, counts] = await Promise.all([
    getDocuments(scope, userId),
    getScopeCounts(userId),
  ]);
            ...
            <CardContent className="p-6">
              {documents.length > 0 ? (
                ...
                <div className="py-12 text-center">
                  ...
                  {scope === "inbox" && (
                    <Button asChild className="mt-6 bg-gradient-to-r from-emerald-600 to-cyan-600 hover:from-emerald-700 hover:to-cyan-700">
